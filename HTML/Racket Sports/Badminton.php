       ...
       <div class="main-container flex-row">
        ...
        <?php
          ...
          while ($row = mysqli_fetch_assoc($products)) {
            ...
            echo '<span class="material-symbols-outlined">star_rate</span>';
            echo '<span class="material-symbols-outlined">star_rate</span>';
            echo '<span class="material-symbols-outlined">star_rate</span>';
            echo '<span class="material-symbols-outlined">star_rate_half</span>';
            echo '</div>';

            echo '<div class="title-price flex-column" >';
            echo '<h3>' . $row['name'] . '</h3>';
            echo '<div>&#x20B9 ' . $row['price'] . '<del>&#x20B9 10000</del></div>';
            echo '</div>';

            echo '<div class="cart-buy ">';
            echo '<form class="add-to-cart flex-column" action="../../PHP/add_to_cart.php" method="post">';
            ...
            // Add size select field if sizes are available
            echo '<div>';  //size-color div
            if (mysqli_num_rows($sizes) > 0) {
              echo '<div class="flex-column" style="width:100%;">';
              echo '<div class="flex-row" style="gap:5px">';
              echo '<select name="size" style="width:50%; color: black;">';
              while ($size = mysqli_fetch_assoc($sizes)) {
                echo '<option value="' . $size['size'] . '" style="color: black;">' . $size['size'] . '</option>';
              }
              echo '</select>';
              echo '</div>';  // Close flex-row div
              echo '</div>';  // Close flex-column div
            }
        

            // Add color select field if colors are available
            if (mysqli_num_rows($colors) > 0) {
              echo '<div class="flex-column" style="width:100%;">';
              echo '<div class="flex-row" style="gap:5px">';
              echo '<select name="color" style="width:50%; color: black;">';
            ...
            echo '</div>';  // Close size-color div


            echo '<div id="cart-button" class="flex-row">';

            echo '<button type ="submit" name="add_to_cart" data-product-id="' . $row['product_id'] . '"><span>Add to Cart</span><span class="material-symbols-outlined">add_shopping_cart</span></button>';
            ...
            echo '</div>';  // Close item-container div
            echo '</div>';  // item-container close

            echo '</div>';  // cart-buy close
